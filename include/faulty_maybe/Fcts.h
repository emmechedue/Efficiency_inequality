...
// Renaormalize array of myMP_float

/*This function renormalizes an array of size N such that its sum amounts to 1*/
void renormalizearray(myMP_float *w, int N){
    myMP_float sum = 0.0;
    int i;
...
// *** Make new function with myMP_float *** // 
/*
myMP_float getutility(int q, double *w, double *alpha, double *O, int *rank, Constants cons, int M){
	int j,k; //i and j are dummy variables, k needs to be updated by hand and will be used for the rank
	myMP_float gain, utility; //The gain in the group and the utlity of q
        ...
        gain=0.0; //Set the gain of the group to 0.
...
void updatestrategy(int k, double *alpha, double *w, double *r,Constants cons, gsl_rng *gslpointer){
    ...
    //double oldutility; //they contain the possible utility for player k for a strategy and the previous one
    //double utility[p]; // Utility array
    //myMP_float oldutility; //they contain the possible utility for player k for a strategy and the previous one
    //myMP_float utility[p]; // Utility array
...
/*
// ***** I MAKE A NEW LOOP HERE: 1) Calculste the array of utilites; 2) normalize it; 3) Calculate the array of probabilites.
...
// *** Array of Utilities *** //
    for(i=0; i < p ; i++){
        ...
        if(place==oldplace){ //If the rank of k is the same, I don't have to reform the groups. I just update the utility
            utility[i] = oldutility + lenghtofinterval * w[k] * (cons.Q * r[k] -1); //It's fine because for i=0, I'm sure that this if will never take place because oldplace is initialized to a negative number
        }
        else{ //In this other case I actually have to reform the group
            utility [i] = getutility(k, w, dummyalpha, dummyO, dummyrank, cons, M); //Actually form the group and compute the utility
        }

	        
        oldutility=utility[i];
        oldplace=place;
}

// *** Normalize the Utilities array *** //
renormalizearray(utility,p);

// *** Now calcualte the probability array *** //
   for(i=0; i < p ; i++){
        //probarr[i] = logitprob(cons.beta , utility); //Here I compute the probability according to the logit and the sum
        //probarr[i] = mpEXPcpp(cons.beta*utility[i]);
	probarr[i] = boost::multiprecision::exp(cons.beta*utility[i]);
	cout << "mpEXP exp(" << cons.beta << "*" << utility[i] << "): " << probarr[i] << endl;
        sum = sum + probarr[i];
    }
*/
