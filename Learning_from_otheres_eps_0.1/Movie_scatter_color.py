...
def main(argv):
	...
	YM = np.amin(w_table)
		...
		# Make area according to the sorting schema 
        	area = np.pi * (4)**2

		if (rankSys == 1):
			area = np.pi * ( (25/(YL*rmax)) * w_table[i] * c_table[i] * r_table[i] + 4)**2
		if (rankSys == 2):
			area = np.pi * ( (10/(YL)) * w_table[i] * c_table[i] + 4)**2
		if (rankSys == 3):
			area = np.pi * ( (10/(rmax)) * c_table[i] * r_table[i] + 4)**2
		if (rankSys == 4):
			area = np.pi * ( (10) * c_table[i] + 4)**2
		...
		# Log scale
		ax.set_yscale('log', nonposy='clip', linthreshy=[YM,-YM] )
