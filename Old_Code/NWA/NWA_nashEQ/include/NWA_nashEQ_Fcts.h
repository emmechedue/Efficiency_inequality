...
// Array of int
void FillSubArray(int *BigArray, int *SubArray, int k, int N){
...
void ensemblePrint(ofstream& filet, ofstream& filew, ofstream& filec, ofstream& fileg, double t, double *eff, double *tot_w, double *w, double *alpha, Constants cons, int N, int NE){
	...
	double* Coop = new double[NE]{};// Average co-op for each society
	...
	fileg << left << setw(6) << t;	
	for (int k=0; k<NE; k++){
		filew << "\t" << left << setw(12) << tot_w[k];
		filec << "\t" << left << setw(12) << Coop[k];
		fileg << "\t" << left << setw(12) << Gini[k];
	}
	filew << endl;
	filec << endl;
	fileg << endl; 
...
// NewStrategy for NWA case !!! 

double NewStrategy_NWA(int k, double *oldalpha, double *w, double *r,Constants cons, gsl_rng *gslpointer){
	...
	place=inverserank(k,dummyrank,cons); //The position where k is placed
        if(place==oldplace){ //If the rank of k is the same, I don't have to reform the groups. I just update the utility
            utility=oldutility + lenghtofinterval * cons.W0 * (cons.Q * r[k] -1); //It's fine because for i=0, I'm sure that this if will never take place because oldplace is initialized to a negative number
        }
        else{ //In this other case I actually have to reform the group
            utility=getutility_NWA(k, w, dummyalpha, dummyO, dummyrank, cons, M); //Actually form the group and compute the utility
        }
