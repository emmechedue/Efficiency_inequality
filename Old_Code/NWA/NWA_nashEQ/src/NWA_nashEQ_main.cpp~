...
int main() {
	...
	double r[N];		// Array to hold INVESTMENT TALENT of each agent
	double w[N];		// Array to hold WEALTH of each agent
	double w0[N];		// Array to hold the INVESTMENT CAP of each agent
		...
		for ( t=1; t<(cons.T+1); t++ ){
			...
			for(int i=0; i<(N); i++){
				...
				for (int i=0; i<N; i++){
					...
					alpha[i] = NewStrategy_NWA(i, oldalpha, w, r, cons, gslpointer);
				}
