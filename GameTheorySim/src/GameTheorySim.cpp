...
// Constructor //
GameTheorySim::GameTheorySim(string inSimName, bool ifSML){
//GameTheorySim::GameTheorySim(){

	// --- Read input parameters from config.conf --- //

	// Type: a bolean name SML 
	SML = ifSML;
...
void GameTheorySim::initOutput(string SimName, bool SML, bool memTest, bool singleP){
	...
	if(SML){
		StrStream << " type=SML/linearPayoff";
	}
	else {
		StrStream << " type=NashEq/linearPayoff";
	}
	...
	// The Single-agent wealth and strategy print file
	if (singleP){
		// Single agent wealth
		SingleW.open(FNSingleW,ios::out|ios::trunc); // Open single agent wealth output file
		if(SingleW.is_open()){
			SingleW << HEAD << "Single agent wealth for each time step for all of the 'societies'" << endl;
			SingleW << PARAM_LIST << endl;
			SingleW << "# Rows: 'Time steps'\tColumns: individuals/players" << endl;
			SingleW << "#" << endl;
		}
		// Single agent cooperation
		SingleC.open(FNSingleC,ios::out|ios::trunc); // Open single agent wealth output file
		if(SingleC.is_open()){
			SingleC << HEAD << "Single agent cooperation for each time step for all of the 'societies'" << endl;
			SingleC << PARAM_LIST << endl;
			SingleC << "# Rows: 'Time steps'\tColumns: individuals/players" << endl;
			SingleC << "#" << endl;
		}
	}
...
void GameTheorySim::runSimulation_NWA(string type, bool MemTest, bool singleP){
	...
	if (type == "SML"){
		...
		for (int scn = 0; scn < NE; scn++){
			...
			// Initialization of the output files //
			initOutput(this->SimName, this->SML, MemTest, singleP);

			// Print parameters
			ensemblePrintParams();

			// Initialise society
			initSociety_SML_NWA(scn, MemTest, singleP);

			// Run the simulation for that society
			runSociety_SML_NWA(scn, MemTest, singleP);
		}
	}
	else if (type == "NashEQ"){
		...
		for (int scn = 0; scn < NE; scn++){
			...
			// Initialise society
			initSociety_NashEQ_NWA(scn, singleP);

			// Run the simulation for that society
			runSociety_NashEQ_NWA(scn, singleP);
		}
...
void GameTheorySim::initSociety_SML_NWA(int k, bool MemTest, bool singleP){
	...
	if (singleP){
		SingleW << "#" << endl;
		SingleW << "#	Society nr. " << k <<  endl;
		SingleW << "#" << endl;
		SingleC << "#" << endl;
		SingleC << "#	Society nr. " << k <<  endl;
		SingleC << "#" << endl;
...
void GameTheorySim::initSociety_NashEQ_NWA(int k, bool singleP){
...
void GameTheorySim::runSociety_SML_NWA(int k, bool MemTest, bool singleP){
...
void GameTheorySim::runSociety_NashEQ_NWA(int k, bool singleP){
...
double GameTheorySim::payoff(double w, double w0, double alpha, double gain){

	return (w - w0*alpha + gain);
...
void GameTheorySim::formgroups_NWA(double *w, double *alpha, double *O, int *rank){
	...
	for(i=0;i<M;i++){ // Here i sum over all the groups
		...
		for(j=0; j<S;j++){
			...
			w[rank[k]] = payoff(w[rank[k]], w0[rank[k]], alpha[rank[k]], gain);
...
double GameTheorySim::getutility_NWA(int q, double *w, double *alpha, double *O, int *rank){
	...
	utility = payoff(w[q], w0[q], alpha[q], gain);
...
double GameTheorySim::NewStrategyNashEQ_NWA(int k){
	...
	place=inverserank(k,dummyrank); //The position where k is placed
		...
		utility = oldutility + payoff(0, w0[k], alpha_interval, alpha_interval*w0[k]*Q*r[k]);
...
void GameTheorySim::memoryUpdate_NWA(){
	...
	// -- The cut-off cap for the memory values -- //
	double CO_up = pow(10, 10);
	double CO_low = pow(10, -10);
	...
	for (int k=0; k<N; k++){
		...
		for (int i=0; i<N_alpha; i++){
			...
			// If the rank of k is the same, I don't have to reform the groups. I just update the utility
				...
				// Using the (k - old_k) is umportant, because due to the if statement about the memory not going 
				// above the cut-off cap, we might skip some of the strategies and k-old_k might not be 1. 
				utility=oldutility + payoff(0, w0[k], alpha_interval, alpha_interval*w0[k]*Q*r[k]);
			...
			double tmp_new_value = tmp_mem*(1.0 + xi[k]*tmp_growth);
			bool HIGH = (tmp_new_value > CO_up);
			bool LOW = (tmp_new_value < CO_low);
			if ( !HIGH && !LOW  ){
				memory[ID] = tmp_new_value;
			}
			else if (HIGH){
				memory[ID] = CO_up;
			}
			...
			// -- Save the oldutility, old_k and oldplace parameters -- //	
			oldutility=utility;
			oldplace=place;
...
// --- Single print -->> The single-agent output --- //
void GameTheorySim::singlePrint(){


	// Printing //
	for (int i=0; i<N; i++){
		SingleW  << left << setw(12) << w[i] << "\t";
		SingleC  << left << setw(12) << alpha[i] << "\t";
	}
	SingleW << endl;
	SingleC << endl;
