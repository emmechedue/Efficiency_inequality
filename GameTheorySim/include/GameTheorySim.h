...
class GameTheorySim {
	...
	GameTheorySim(string SimName, bool ifSML);	// IF SML == TRUE -->> Simple memory learning simulation ELSE Nash eq.
	//GameTheorySim();	
	~GameTheorySim();

	// Initialization of the output files //
	virtual void initOutput(string SimName, bool SML, bool memTest, bool singleP);

	// The society initialization method for simple memory learning (SML) schmea //
	virtual void initSociety_SML_NWA(int k, bool MemTest, bool singleP);

	// The society initialization method for Nash Equlibrium schmea //
	virtual void initSociety_NashEQ_NWA(int k, bool singleP);

	// Run simulation for one full society for SML //
	virtual void runSociety_SML_NWA(int k, bool MemTest, bool singleP);

	// Run simulation for one full society for NashEQ //
	virtual void runSociety_NashEQ_NWA(int k, bool singleP);

	// Run Full simulation: either SML or NashEQ //
	virtual void runSimulation_NWA(string type, bool MemTest, bool singleP);
	...
	ofstream SingleW;	// Single agent wealth output stream
	ofstream SingleC;	// Single agent cooperation/strategy output stream
	...
	const string FNSingleW = string ("singleAgent_Wealth.txt");	// File to hold the single agent wealth output for each timestep
	const string FNSingleC = string ("singleAgent_Cooperation.txt");// File to hold the single agent cooperation output for each timestep
	...
	virtual double payoff(double w, double w0, double alpha, double gain);

	// The makeorder mehtod, which ALSO fills the OUTPUT array //
	virtual void makeorder_NWA(double *alpha, int *rank, double *O);		

	// The GROUPING methods, which ALSO claculate the UTILITY //
	virtual void formgroups_NWA(double *w, double *alpha, double *O, int *rank);	
	...
	virtual double getutility_NWA(int q, double *w, double *alpha, double *O, int *rank);
	...
	virtual double NewStrategyNashEQ_NWA(int k);						// Nash equlibrium
	//double NewStrategyNashEQ_NWA(int k);						// Nash equlibrium
	virtual double newStrategySML_NWA(int k); 						// Simple learning schema 

	// Memory update methods for the simple learning schema //
	virtual void memoryUpdate_NWA(); 
