...
void makeorder_NWA(double *r,double w,double *alpha,int *rank,Constants cons, double *O){
...
void formgroups_NWA(double *w, double *alpha, double *O, int *rank, Constants cons, int M){
	int i,j,k; //i and j are dummy variables, k needs to be updated by hand and will be used for the rank
	double gain; //The gain in the group
	...
	for(i=0;i<M;i++){ //Here i sum over all the groups
		...
		for(j=0; j<cons.S;j++){
			...
			w[rank[k]] = w[rank[k]] - cons.W0*alpha[rank[k]] + gain; //Here I update the wealth of all the players
			k++; //Here I update k so that it keeps track for the next round
		}
...
void printparamsingleloop(ofstream& filec, Constants cons){
	...
	filec << "#The coef. for memory update" << endl;
	filec << "mUpDate= " << cons.mUpDate << endl << endl;
...
void printstuffsingleloop(ofstream& filet,ofstream& filew, ofstream& filec ,int t, double eff, double *w, double *alpha,Constants cons){
    ...
    filet << left << setw(6) << t <<"\t" << left << setw(10) << gini << "\t";
    filet << left << setw(10) << eff << "\t" << left << setw(10) << averagec << endl; //Here I print the time, the Gini coefficient, the efficiency and the average level of cooperation
    ...
    for(i=0;i<cons.N;i++){ //Here I print the entire wealth for each agent at each time step and (on another file) the strategy of each agent
        filew << left << setw(10) << w[i] << "\t";
        filec << left << setw(10) << alpha[i] << "\t";
    }
    filew<<endl;
    filec<<endl;
    return;
...
void printMemory(ofstream& filem, int t, int N, int N_alpha, double *memory){
...
// ----- New update strategy for simple_memory_learning algorithm ----- //
int newStrategySML(int k, double *memory, int N_alpha, Constants cons, gsl_rng *gslpointer){
	...
	for(int i=0; i < N_alpha ; i++){
		...
		//Here I compute the probability according to the logit and the sum
	    	// ***** I use mpEXP from myEXP.h for taking the exponent ***** //
	    	probarr[i] = mpEXP(cons.beta*memory[ID]);
	    	sum = sum + probarr[i];
	...
	int out = binaryprobsearch(&cumprob[0],N_alpha,RN); 
	
	//double out = i*lenghtofinterval; //Here I set the value for alpha[k]. I.e. I set the strategy for agent k
...
void memoryUpdate(double *memory, double *alpha,  double *OLDw, double *r, int N_alpha, Constants cons){
		...
		for (int i=0; i<N_alpha; i++){
			...
			if(!(place==oldplace)){ 
				...
				// Form the group and compute the utility
            			utility=getutility(k, OLDw, dummyalpha, dummyO, dummyrank, cons, M); 
        		}
			// If the rank of k is the same, I don't have to reform the groups. I just update the utility
		        else{ 
		             utility=oldutility + lenghtofinterval * OLDw[k] * (cons.Q * r[k] -1);
        		}
